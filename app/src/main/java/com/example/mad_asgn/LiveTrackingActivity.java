...
public class LiveTrackingActivity extends AppCompatActivity implements OnMapReadyCallback {
    ...
    private CardView safetyCheckCard;
    private Button confirmSafetyButton;
    private Button notSafeButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        safetyCheckCard.setVisibility(View.GONE);
    ...
    private void initializeViews() {
        ...
        safetyCheckCard = findViewById(R.id.safetyCheckCard);
        confirmSafetyButton = findViewById(R.id.confirmSafetyButton);
        notSafeButton = findViewById(R.id.notSafeButton);
    ...
    private void setupButtonListeners() {
        ...
        confirmSafetyButton.setOnClickListener(v -> {
            safetyCheckCard.setVisibility(View.GONE);
            isVehicleStopped = false;
            userConfirmedSafe = true;  // User confirmed they're safe, no more safety checks
            handler.removeCallbacksAndMessages(null);  // Remove any pending safety check countdowns
            startTracking();  // Resume tracking without safety checks
        });

        // Set up not safe button
        notSafeButton.setOnClickListener(v -> showSOSConfirmation(true));
        ...
        if (!userConfirmedSafe) {
            handler.postDelayed(() -> {
                if (isVehicleStopped && !userConfirmedSafe) {
                    safetyCheckCard.setVisibility(View.VISIBLE);
                    startSafetyCheckCountdown();
                }
    ...
    private void startSafetyCheckCountdown() {
        final int[] countdown = {30}; // 30 seconds countdown
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                // Stop countdown if user confirmed they're safe
                if (userConfirmedSafe) {
                    return;
                }
                ...
                countdown[0]--;
                countdownText.setText(countdown[0] + " seconds to respond");
